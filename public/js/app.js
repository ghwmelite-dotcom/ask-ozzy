...
async function loadChartJs() {
  ...
  await _loadScript('https://cdn.jsdelivr.net/npm/chart.js@4.4.7/dist/chart.umd.min.js');
}

async function loadDocxJs() {
  if (typeof docx !== 'undefined') return;
  await _loadScript('https://cdn.jsdelivr.net/npm/docx@9.1.1/build/index.umd.min.js');
}
...
function renderConvoItem(c, inFolder) {
  ...
  const safeFolderId = c.folder_id ? c.folder_id.replace(/'/g, "\\'") : null;
  return `
    <div class="conversation-item ${c.id === state.activeConversationId ? "active" : ""} ${c.pinned ? "pinned" : ""}"
         onclick="openConversation('${safeId}')" oncontextmenu="showConvoContextMenu(event,'${safeId}',${!!c.pinned},${safeFolderId ? `'${safeFolderId}'` : 'null'})" ${inFolder ? 'style="padding-left:28px;"' : ""}>
      <span class="convo-icon">${c.pinned ? "📌" : "💬"}</span>
      <span class="convo-title">${escapeHtml(c.title)}</span>
      <div class="convo-actions">
        ${isPaid ? `<button class="convo-action-btn" onclick="event.stopPropagation();showMoveToFolderMenu(event,'${safeId}')" title="Move to folder">📁</button>` : ""}
        <button class="convo-action-btn" onclick="event.stopPropagation();togglePin('${safeId}')" title="${c.pinned ? 'Unpin' : 'Pin'}">${c.pinned ? "📌" : "📍"}</button>
        <button class="convo-action-btn convo-delete" onclick="event.stopPropagation();deleteConversation('${safeId}')" title="Delete">🗑</button>
      </div>
    </div>`;
}
...
async function sendMessage() {
  ...
  try {
    ...
    // Add streaming class to materializing message
    const streamingMsgEls = document.querySelectorAll('.message.assistant');
    const streamingEl = streamingMsgEls[streamingMsgEls.length - 1];
    ...
    // Gold dust completion burst on last assistant message
    const completedEls = document.querySelectorAll('.message.assistant');
    const completedEl = completedEls[completedEls.length - 1];
    if (completedEl) {
      completedEl.classList.add('completed');
      setTimeout(() => completedEl.classList.remove('completed'), 1200);
    }
...
function renderMarkdown(text) {
  ...
  // Tables (on escaped text, pipe chars are literal since they're not HTML-special)
  html = html.replace(
    /\|(.+)\|\n\|[-| ]+\|\n((?:\|.+\|\n?)*)/g,
    (_, header, rows) => {
  ...
  // Smart card wrappers
  html = html.replace(/<pre><code(.*?)>([\s\S]*?)<\/code><\/pre>/g, (match, attrs) => {
    const langMatch = attrs.match(/class="language-(\w+)"/);
    const lang = langMatch ? langMatch[1] : '';
    const label = lang ? `<span class="code-card-label">${lang}</span>` : '';
    return `<div class="smart-card code-card">${label}${match}</div>`;
  });
  html = html.replace(/(<ul>[\s\S]*?<\/ul>)/g, '<div class="smart-card list-card">$1</div>');
  html = html.replace(/(<blockquote>[\s\S]*?<\/blockquote>)/g, '<div class="smart-card quote-card">$1</div>');
...
function showMoveToFolderMenu(event, convoId) {
  ...
  if (convo && convo.folder_id) {
    menuHtml += `<button class="context-menu-item" onclick="moveToFolder('${safeConvoId}',null);closeContextMenu();">Remove from folder</button>`;
  }
...
function showConvoContextMenu(event, convoId, isPinned, folderId) {
  ...
  menuHtml += '<div class="context-menu-divider"></div>';
  menuHtml += `<button class="context-menu-item context-menu-danger" onclick="deleteConversation('${safeConvoId}');closeContextMenu();">🗑 Delete</button>`;
  menuHtml += '</div>';
  ...
  if (navigator.serviceWorker) {
    navigator.serviceWorker.addEventListener('message', function (e) {
      if (e.data && e.data.type === 'NAVIGATE') {
        try {
          const navUrl = new URL(e.data.url, location.origin);
          if (navUrl.origin === location.origin) window.location.href = navUrl.href;
        } catch {}
      }
    });
  }
...
(function captureReferralFromURL() {
  ...
  openAuthModal = function () { // eslint-disable-line no-func-assign
    origOpen();
...
function renderAgentSelector() {
  ...
  // Add agent selector as tool-pill in input-tools row
  ...
  if (!agentBtn) {
    ...
    agentBtn.className = 'tool-pill agent-pill';
    agentBtn.onclick = openAgentModal;
    agentBtn.title = 'Choose a specialized AI agent';
    const inputTools = document.querySelector('.input-tools');
    if (inputTools) inputTools.insertBefore(agentBtn, inputTools.firstChild);
  }

  const active = state.selectedAgent;
  const robotSvg = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="10" rx="2"/><circle cx="12" cy="5" r="2"/><line x1="12" y1="7" x2="12" y2="11"/><circle cx="8" cy="16" r="1"/><circle cx="16" cy="16" r="1"/></svg>';
  if (active) {
    const agent = agents.find(a => a.id === active);
    agentBtn.innerHTML = `${robotSvg} ${agent ? agent.name : 'Agent'}`;
    agentBtn.classList.add('active');
  } else {
    agentBtn.innerHTML = `${robotSvg} Agents`;
...
function renderWorkflowStep(container, workflowId, steps, name, stepIndex) {
  ...
  const html = `<div class="workflow-wizard">
